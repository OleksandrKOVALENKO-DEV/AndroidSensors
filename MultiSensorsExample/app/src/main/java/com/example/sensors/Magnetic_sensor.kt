...
// Клас Magnetic_sensor, який успадковує клас AppCompatActivity
class Magnetic_sensor : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val text_magnetic_data: TextView = findViewById(R.id.magnetic_data)
        ...
        // Отримання об'єкта сенсора магнітного поля з доступних сенсорів
        val sensor = sensor_manager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD)
        ...
        val sensor_listener = object: SensorEventListener {
            ...
            override fun onSensorChanged(event: SensorEvent?) {
                ...
                // Форматування значень по осях X, Y, Z з точністю до двох знаків після коми
                val formattedX = "%.2f".format(value?.get(0))
                val formattedY = "%.2f".format(value?.get(1))
                val formattedZ = "%.2f".format(value?.get(2))
                // Формування рядка зі значеннями магнітного поля
                val sensor_data = "X: $formattedX µT\nY: $formattedY µT\nZ: $formattedZ µT"
                // Оновлення тексту у TextView
                text_magnetic_data.text = sensor_data
            }
